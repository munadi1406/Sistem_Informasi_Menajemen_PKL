...
import Form from "./suratKeluar/Form";
...
export default function SuratKeluar() {
  ...
  const {
    ...
    data,
    refetch,
  } = useInfiniteQuery(`listSuratKeluar`, {
    queryFn: async ({ pageParam }) => {
      const data = await getListSuratKeluar(pageParam || 0, ``);
      return data.data;
    },
  ...
  const handleGetDetailSuratKeluar = useQuery(
    `detailSuratKeluar-${currentId}`,
    {
      queryFn: async () => {
        const datas = await detailSuratKeluar(currentId);
  ...
  const handleDeleteSurat = useMutation({
    mutationFn: async () => {
      const data = await deleteSuratKeluar(currentId);
      return data.data;
    },
    onSuccess: (data) => {
  ...
  const handleSearchSuratKeluar = useInfiniteQuery(
    `listSearchSuratKeluar-${query}-${filter.startDate}-${filter.endDate}`,
    {
      queryFn: async ({ pageParam }) => {
        const data = await getListSuratKeluar(
          pageParam || 0,
          `${
            isSearch
              ? `?search=${query}${
  ...
  return (
    ...
    <Helmet title={"Surat Keluar"} />
      <Form
        ...
        title={openModalEdit ? "Edit Surat Keluar" : "Tambah Surat Keluar"}
        refetch={refetch}
        isEdit={openModalEdit}
        dataSuratKeluar={handleGetDetailSuratKeluar.data}
        currentId={currentId}
      />
      <ModalDeleteSuratKeluar
          ...
          <div className="mb-8 flex items-center justify-between gap-8">
            ...
            <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
              ...
              <ButtonCustom
                ...
                text={
                  <>
                    <FaPlusCircle className="h-4 w-4" /> Tambah Surat Keluar
                  </>
                }
                color="blue"
          ...
          <div className="flex flex-col items-start justify-center gap-4 w-full">
            <div className="w-full md:w-72">
              <TextInput
                label="Search"
                icon={
                  handleSearchSuratKeluar.isLoading ? (
                    <Spinner />
                  ) : (
        ...
        <CardBody className="overflow-auto px-0 min-h-screen">
          <Suspense fallback={<TableSkeleton />}>
            <DataSuratKeluar
              dataSurat={
                isSearch || isFilter
                  ? handleSearchSuratKeluar.data
                    ? handleSearchSuratKeluar.data.pages
                    : data.pages
                  : data.pages
              }
