...
const Form = ({ handleSubmit, errorMsg, isEdit, data }) => {
    ...
    const handleSub = (e) => {
        e.preventDefault()
        handleSubmit.mutate({ jenisSurat, variable: variabel, isiTemplate: templateContent })
    }
            ...
            <div className='flex flex-col gap-2'>
                {variableSetting.map((e, i) => (
                    <div className='flex justify-center items-center gap-2' key={i}>
                        ...
                        <Select label='Tipe Data' color='blue' >
    ...
    useEffect(() => {
        if (templateContent.length > 0) {
            ...
            if (hasil) {
                ...
                const newVariables = result.map(variableName => ({ [variableName]: '' }));
                setVariabel([...variabel, ...newVariables]);
            } else {
                setVariableSetting([]);
            }
        }
